  ...
  try {
    const opts = {
      ...
      headers: {
        cookie: user.cookie,
        "user-agent":
          "Mozilla/5.0 (iPhone; CPU iPhone OS 18_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.57(0x18003930) NetType/WIFI Language/zh_CN miniProgram/wx50282644351869da",
        token: user.token,
  ...
  try {
    const opts = {
      ...
      headers: {
        cookie: user.cookie,
        "user-agent":
          "Mozilla/5.0 (iPhone; CPU iPhone OS 15_8 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.48(0x18003029) NetType/4G Language/zh_CN miniProgram/wx50282644351869da",
    ...
    let res = await fetch(opts);
    $.log(
      `${$.doFlag[res?.code == "0000"]} ${res?.code == "0000" ? "抽奖签到: 成功, 获得" + res?.data?.ticket_times + "次抽奖机会" : "抽奖签到: " + res?.message}\n`,
    );
  } catch (e) {
    $.log(`⛔️ 抽奖签到失败！${e}\n`);
  }
...
async function lotteryClock(user) {
  try {
    ...
    let res = await fetch(opts);
    $.log(
      `${$.doFlag[res?.code == "0000"]} ${res?.code == "0000" ? "抽奖成功, 获得" + res?.data?.desc : "抽奖: " + res?.message}\n`,
    );
  } catch (e) {
    $.log(`⛔️ 抽奖失败！${e}\n`);
  }
